...
- `apiEnv` (string, optional): Specifies which API environment to use for the chatbot. Possible values are `'demo'` and `'prod'`. If set to `'prod'`, the production API will be used. If set to `'demo'` or left unspecified, the demo API will be used by default. Example: `apiEnv: 'prod'` for production, `apiEnv: 'demo'` for demo (default).
